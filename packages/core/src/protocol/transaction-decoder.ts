...
export default class TransactionDecoder {
    ...
    //     const sender: Uint16Array = this.getSender(txn);
    ...
    public decodeTransfer(
        ...
        const amountBigInt = dataView.getBigInt64(6, false); // Read amount as BigInt for precision
        const amount = Number(amountBigInt) / 1_000_000_000;
    ...
    //     const senderHex = `0x${byteArrayToHex(sender)}`;
    ...
    //     return {
    //         sender: `0x${byteArrayToHex(sender)}`,
    //         nonce: nonce,
    //         size: txn.length,
    //         rawTransaction: txn,
    //         chainId: txn[1],
    ...
    //     const externalVmId = dataView.getBigUint64(6, false); // Assuming little-endian; adjust as needed

    //     // Check if sender is a VM address to determine how to calculate data length
    //     const senderHex = `0x${bytesToHex(sender)}`;
    //     const isVm = PWRJS.isVmAddress(senderHex);

    //     const baseLength = isVm ? 14 : 79; // adjust base length based on the type of address
    //     if (txn.length <= baseLength) {
    //         throw new Error(
    //             `Data segment calculation error: Transaction length is insufficient for expected components (length: ${txn.length}, base required: ${baseLength})`
    //         );
    //     }

    //     const dataLength = txn.length - baseLength;

    //     // Extract data starting right after externalVmId
    //     const dataStart = 14; // After identifier (1), chain id (1), nonce (4), and external VM ID (8)
    ...
    // static getSigner(txn: Uint8Array, signature: Uint8Array): Uint8Array {
    ...
    // private static decodeJoin(
    //     txn: Uint8Array,
    //     sender: Uint8Array
    // ): Transaction {}

    // private static decodeClaimSpot(
    //     txn: Uint8Array,
    //     sender: Uint8Array
    // ): Transaction {}
    ...
    public decodeSetGuardian(
        ...
        if (txn.length != 99 && txn.length != 34)
        ...
        // get the expiry date
        const expiryDate = dataView.getBigInt64(6, false);
