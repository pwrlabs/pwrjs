...
describe('', () => {
    ...
    test('test service itself', async () => {
        ...
        const signature = await service.sign('hello', keypair.pk, keypair.sk);

        const valid = await service.verify('hello', keypair.pk, signature);
    ...
    test('test signature altered', async () => {
        ...
        const valid2 = await service.verify(
            javaSign.message,
            { H: 'fake' },
            javaSign.signature
        );
    ...
    test('test fail', async () => {
        ...
        const signature = await service.sign('hello', fakepk, keypair.sk);
    });
